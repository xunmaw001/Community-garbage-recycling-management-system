...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussfeipinhuishouCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussfeipinhuishouCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussfeipinhuishou');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussfeipinhuishou');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussfeipinhuishou', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussfeipinhuishou', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussfeipinhuishou/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussfeipinhuishou', this.detail);
				this.$utils.msg('审核成功');
